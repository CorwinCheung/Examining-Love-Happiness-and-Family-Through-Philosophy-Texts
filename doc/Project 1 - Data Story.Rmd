...
Let's first explore these partitioned data sets via Wordclouds. WordClouds are a great visualization tool to see which words are most prevelant in the selected sentences we picked with each section. This data story removed stopwords and the words that are filtered for so that the most important words are displayed. 
...
These word clouds are great, and capture the most common words in family, happiness, and family datasets. Now, we'll analyze using word embeddings. By fitting a model to the sentence data, we get a word embedding that can predict which terms are the most similar to any other word in the set. For Love: the closest words are Jove, conceit, kindness, aversion, and complacency. For happiness, the closest 5 words are appetite, inclination, wickedness, satisfaction, and pride. And for family, the closest 5 words are tribe, valour, kingdom, apparel, and nation(all using cosine similarity functions). Looking at the closest words for each of the prevalent word clouds gives us a web of how the terms are related in these philosophical texts. For example we see that the love and happiness topics both talk about good as an important word. and family and love both have woman as an important word. Looking at the dataframe closest words for each of the important wordclouds, we can see more similarities between the important words of the data.
